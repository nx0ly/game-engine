...
class MathUtils implements UtilsStruct {
    ...
    multiplyVector(matrix: Matrix, vector: Vector): Vector {
        return {
            x: matrix.elements[0][0] * vector.x + matrix.elements[0][1] * vector.y + matrix.elements[0][2] * vector.z,
            y: matrix.elements[1][0] * vector.x + matrix.elements[1][1] * vector.y + matrix.elements[1][2] * vector.z,
            z: matrix.elements[2][0] * vector.x + matrix.elements[2][1] * vector.y + matrix.elements[2][2] * vector.z,
        };
    }
    ...
    determinantMatrix(matrix: Matrix): number {
        ...
        for (let i = 0; i < matrix.elements.length; i++) {
            ...
            sum += matrix.elements[0][i] * this.determinantMatrix(minor) * (i % 2 === 0 ? 1 : -1);
        }
                ...
                for (let k = 0; k < matrix.elements.length; k++) {
                    if (k !== j) {
                        ...
                        for (let l = 0; l < matrix.elements.length; l++) {
                            if (l !== i) {
                                minor.elements[minor.elements.length - 1].push(matrix.elements[k][l]);
                            }
                        }
                    }
                }
                inverseMatrix.elements[i].push(this.determinantMatrix(minor) * ((i + j) % 2 === 0 ? 1 : -1) / determinant);
            }
        }
